 ...
 * @returns {false|undefined} False when the project state cannot be loaded.
 */
function runExport(cwd = process.cwd(), options = {}) {
  ...
  if (!state) {
    logger.error('Cannot export documentation without valid project state. Restore .buildwithai/state.json from a backup.');
    return false;
  }
